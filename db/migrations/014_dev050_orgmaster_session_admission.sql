...
-- contract-impact: additive OrgMaster app-scoped session admission projection
-- compatibility: additive
-- governance-review: DEV-050
...
CREATE VIEW orgmaster_contract.v_orgmaster_session_principals_v1
...
WITH active_governance AS (
  ...
  WHERE assignment.value->>'applicationId' = 'orgmaster'
    ...
    AND assignment.value#>>'{scope,kind}' = 'global'
    ...
    AND EXISTS (
      ...
      WHERE application.value->>'id' = 'orgmaster'
        AND application.value->>'status' = 'active'
    )
)
SELECT
  'orgmaster.session-principal.v1'::text AS contract_version,
...
JOIN eligible_employees eligible ON eligible.employee_id = mapping.employee_id
WHERE mapping.employee_status = 'active';

ALTER VIEW orgmaster_contract.v_orgmaster_session_principals_v1 OWNER TO jenfu_orgmaster_migrator;
REVOKE ALL ON orgmaster_contract.v_orgmaster_session_principals_v1 FROM PUBLIC, jenfu_platform_runtime, jenfu_ai_pdm_runtime;
GRANT SELECT ON orgmaster_contract.v_orgmaster_session_principals_v1 TO jenfu_orgmaster_runtime;
