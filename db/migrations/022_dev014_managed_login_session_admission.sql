...
SELECT pg_advisory_xact_lock(
  hashtext('dev014-managed-login-session-admission'),
...
CREATE VIEW orgmaster_contract.v_orgmaster_session_principals_v2
  ...
  WHERE assignment.value->>'applicationId' IN ('orgmaster', 'ai-pdm')
    ...
    AND EXISTS (
      ...
      FROM jsonb_array_elements(
        CASE WHEN jsonb_typeof(version.payload->'applicationRoles') = 'array'
          THEN version.payload->'applicationRoles' ELSE '[]'::jsonb END
      ) role(value)
    ...
    AND EXISTS (
      SELECT 1
      FROM jsonb_array_elements(
        CASE WHEN jsonb_typeof(version.payload->'applications') = 'array'
          THEN version.payload->'applications' ELSE '[]'::jsonb END
      ) application(value)
