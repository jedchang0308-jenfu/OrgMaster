...
describe('DEV-010 N2 OrgMaster database boundary', () => {
  ...
  it('routes identity, auth epoch and invalidation through producer contracts', async () => {
    const query = vi.fn(async (sql: string) => {
      ...
      if (sql.includes('v_orgmaster_session_principals_v2')) return { rows: [{ contract_version: 'orgmaster.session-principal.v2', principal_issuer: 'issuer', principal_subject: 'subject', principal_id: 'principal', employee_id: 'employee', employee_status: 'active', mapping_version: 1, published_at: '2026-09-03T00:00:00.000Z' }], rowCount: 1 }
    ...
    expect(sql).toContain('orgmaster_contract.v_orgmaster_session_principals_v2')
