...
function decodeTx(value: string | null, secret: string): Tx | null {
  ...
  try {
    ...
    if (tx.clientId !== 'orgmaster' || !/^[A-Za-z0-9_-]{22,256}$/u.test(tx.state) || !/^[A-Za-z0-9_-]{43,128}$/u.test(tx.verifier) || !Number.isSafeInteger(tx.expiresAt) || tx.expiresAt < Date.now()) return null
...
function baseConfig(runtime: OrgmasterAuthRuntime) {
  ...
  if (broker.protocol !== 'https:' && !(process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') ) throw new Error('sso broker must use https')
...
async function serviceToken(audience: string) {
  const auth = new GoogleAuth()
  const client = await auth.getIdTokenClient(audience)
  return client.idTokenProvider.fetchIdToken(audience)
}

function parseHandoff(value: unknown, expectedIssuer: string): Handoff {
  ...
  if (![authenticatedAt, sourceExpiresAt, expiresAt].every(Number.isFinite) || authenticatedAt > Date.now() + 60_000 || expiresAt <= Date.now() || sourceExpiresAt <= Date.now()) throw new Error('handoff expired')
...
export async function handleOrgmasterSsoRequest(request: IncomingMessage, response: ServerResponse, runtime: OrgmasterAuthRuntime, id: string) {
  ...
  let setup
  try { setup = baseConfig(runtime) } catch { error(response, 503, 'sso_server_not_configured', id); return true }
  ...
  if (url.pathname.endsWith('/start')) {
    ...
    const tx: Tx = { state, verifier, returnTo, issuer: setup.issuer, clientId: 'orgmaster', expiresAt: Date.now() + 300_000 }
  ...
  if (request.method !== 'GET') { error(response, 405, 'sso_request_invalid', id); return true }
  const tx = decodeTx(cookieValue(request.headers.cookie), setup.config.sessionHashPepper)
  ...
  try {
    const token = await serviceToken(setup.brokerOrigin)
    ...
    const brokerResponse = await fetch(`${setup.brokerOrigin}/api/sso/token`, { method: 'POST', redirect: 'error', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Jenfu-Service-Identity': `Bearer ${token}` }, body })
    if (!brokerResponse.ok) throw new Error('exchange failed')
    const handoff = parseHandoff(await brokerResponse.json(), setup.issuer)
    ...
    response.setHeader('Referrer-Policy', 'no-referrer')
    response.setHeader('X-Correlation-Id', id)
    response.setHeader('Set-Cookie', [sessionCookie(localToken, setup.config.secureCookie, Math.max(1, Math.floor((maxExpiry - now) / 1000))), clearCookie(setup.config.secureCookie)])
    response.end()
  } catch (errorValue) {
