...
describe('OrgMaster session principal admission', () => {
  it('reads the app-scoped view and requires its contract version', async () => {
    ...
    expect(query.mock.calls[0][0]).toContain('v_orgmaster_session_principals_v1')
  ...
  it('rejects the shared or malformed contract instead of falling back', async () => {
    const query = vi.fn(async () => ({ rows: [{ contract_version: 'organization.active-principal.v1', principal_issuer: 'issuer', principal_subject: 'subject', principal_id: 'principal', employee_id: 'employee', employee_status: 'active', mapping_version: '42', published_at: '2026-09-17T00:00:00.000Z' }] }))
