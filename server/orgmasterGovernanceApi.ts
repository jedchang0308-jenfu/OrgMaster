...
import { EmployeeAuthoritySwitchError, switchEmployeeEntitlementAuthority, type EmployeeAuthoritySwitchRequest } from './employeeAuthoritySwitchStore'
import { isAllowedOrgmasterRequestOrigin } from './orgmasterAuthApi'
...
export function governanceErrorStatus(code: string) { if (code === 'IDENTITY_CONTEXT_REQUIRED' || code === 'IDENTITY_REQUIRED') return 401; if (code === 'IDENTITY_PROVIDER_NOT_CONFIGURED' || code === 'EXTERNAL_CATALOG_UNAVAILABLE' || code === 'ROLE_CAPABILITY_READ_FAILED' || code === 'ORGMASTER_SOURCE_UNAVAILABLE' || code === 'SECURITY_ALERT_PERSIST_FAILED' || code === 'FINANCIAL_CATALOG_UNAVAILABLE' || code === 'CATALOG_REGISTRY_UNAVAILABLE' || code === 'ENTITLEMENT_AUTHORITY_SWITCH_FAILED' || code === 'authority_transaction_mode_invalid') return 503; if (['GOVERNANCE_ADMIN_REQUIRED', 'GOVERNANCE_PUBLISH_REQUIRED', 'ROLE_POSITION_ADOPTION_FORBIDDEN', 'PRIVILEGED_OVERRIDE_REQUIRED', 'PRIVILEGED_VIEW_REQUIRED', 'PRIVILEGED_MUTATION_REQUIRED', 'PRIVILEGED_SELF_ASSIGNMENT_DENIED', 'STEP_UP_REQUIRED', 'APP_MANAGEMENT_GRANT_REQUIRED', 'APP_PUBLISH_GRANT_REQUIRED', 'PRIVILEGED_ADMIN_REQUIRED', 'ENTITLEMENT_AUTHORITY_SWITCH_FORBIDDEN', 'ENTITLEMENT_AUTHORITY_SYSTEM_ADMIN_REQUIRED'].includes(code)) return 403; if (['POLICY_VERSION_NOT_FOUND', 'ROLE_NOT_FOUND', 'PRIVILEGED_ASSIGNMENT_NOT_FOUND', 'EMPLOYEE_NOT_FOUND', 'PRINCIPAL_NOT_FOUND', 'COMMAND_NOT_FOUND', 'ASSIGNMENT_NOT_FOUND', 'IDENTITY_LINK_NOT_FOUND', 'ENTITLEMENT_AUTHORITY_STATE_NOT_FOUND'].includes(code)) return 404; if (code === 'PAYLOAD_TOO_LARGE') return 413; if (['REVISION_CONFLICT', 'COMMAND_ID_REUSED', 'COMMAND_STILL_PROCESSING', 'COMMAND_PROCESSING_LEASE_EXPIRED', 'COMMAND_NOT_OBSERVED', 'COMMAND_ALREADY_TERMINAL', 'REQUEST_HASH_MISMATCH', 'PREVIEW_HASH_MISMATCH', 'CATALOG_VERSION_CONFLICT', 'CATALOG_PAYLOAD_HASH_MISMATCH', 'ORGANIZATION_VERSION_INVALID', 'EXTERNAL_CATALOG_READ_ONLY', 'EXTERNAL_CATALOG_VERSION_CONFLICT', 'EXTERNAL_CATALOG_STALE', 'LEGACY_MIGRATION_UNRESOLVED', 'LEGACY_POLICY_REACTIVATION_FORBIDDEN', 'GOVERNANCE_V3_MIGRATION_REQUIRED', 'GOVERNANCE_REVISION_CONFLICT', 'ORGANIZATION_REVISION_CONFLICT', 'CATALOG_BINDING_CONFLICT', 'PREVIEW_HASH_CONFLICT', 'REQUEST_HASH_CONFLICT', 'COMMAND_ID_CONFLICT', 'IDENTITY_LINK_CONFLICT', 'IDENTITY_ACCOUNT_FLOW_REQUIRED', 'ENTITLEMENT_AUTHORITY_VERSION_CONFLICT', 'ENTITLEMENT_AUTHORITY_OPERATION_REUSED', 'ENTITLEMENT_AUTHORITY_ACTIVE_VERSION_REQUIRED', 'ENTITLEMENT_AUTHORITY_SOURCE_UNCHANGED', 'legacy_receipt_unbound'].includes(code)) return 409; if (['GOVERNANCE_VALIDATION_FAILED', 'EXTERNAL_CATALOG_INVALID', 'EXTERNAL_ROLE_UNKNOWN', 'EXTERNAL_ROLE_INACTIVE', 'EXTERNAL_ROLE_UNASSIGNABLE', 'EXTERNAL_SCOPE_UNSUPPORTED', 'ROLE_DELEGATION_INVALID', 'PUBLISHER_NOT_LINKED', 'GOVERNANCE_ADMIN_CONTINUITY_REQUIRED', 'POSITION_NOT_FOUND', 'POSITION_NOT_ADOPTED', 'PRIVILEGED_PRINCIPAL_REQUIRED', 'CATALOG_ROLE_CONTRACT_MISMATCH', 'UNSUPPORTED_SUBJECT', 'UNSUPPORTED_SCOPE', 'INVALID_VALIDITY', 'CATALOG_INCOMPATIBLE', 'SELF_GRANT_FORBIDDEN', 'GOVERNANCE_CONTINUITY_REQUIRED', 'DUPLICATE_ACTIVE_ASSIGNMENT', 'EMPLOYEE_NOT_ACTIVE', 'SELF_IDENTITY_LINK_DEACTIVATION_FORBIDDEN', 'IDENTITY_LINK_ACTIVE_ADMISSION_FORBIDDEN', 'ENTITLEMENT_AUTHORITY_EMPLOYEE_INACTIVE', 'ENTITLEMENT_AUTHORITY_TARGET_IDENTITY_INVALID', 'ENTITLEMENT_AUTHORITY_ASSIGNMENT_INVALID'].includes(code)) return 422; if (['INVALID_JSON', 'INVALID_COMMAND', 'ENTITLEMENT_AUTHORITY_SWITCH_INPUT_INVALID'].includes(code)) return 400; if (code === 'LEGACY_GOVERNANCE_EVALUATOR_RETIRED' || code === 'CHANGE_CURSOR_EXPIRED') return 410; return 500 }
function errorResponse(response: ServerResponse, error: unknown) { const applicationError = error instanceof ApplicationEntitlementError; const code = applicationError ? error.code : error instanceof EmployeeAuthoritySwitchError ? error.code : error instanceof GovernanceStoreError ? error.code : error instanceof AiPdmRoleCapabilityStoreError ? error.code : error instanceof PrivilegedAssignmentError ? error.code : error instanceof GovernanceValidationError ? 'GOVERNANCE_VALIDATION_FAILED' : 'GOVERNANCE_READ_FAILED'; if (governanceErrorStatus(code) >= 500) console.error(JSON.stringify({ event: 'orgmaster_governance_request_failed', code, details: error instanceof GovernanceStoreError ? error.details ?? null : null })); if (applicationError) { const retryable = ['FINANCIAL_CATALOG_UNAVAILABLE', 'CATALOG_REGISTRY_UNAVAILABLE', 'GOVERNANCE_REVISION_CONFLICT', 'ORGANIZATION_REVISION_CONFLICT'].includes(code); sendJson(response, governanceErrorStatus(code), { code, message: '目前無法完成權限治理操作。', correlationId: `corr-${randomUUID()}`, ...(retryable ? { retryable: true } : {}) }); return } sendJson(response, governanceErrorStatus(code), { error: code, ...(error instanceof GovernanceValidationError ? { issues: error.issues } : {}) }) }

function requireGovernanceMutationOrigin(request: IncomingMessage) {
  const config = readOrgmasterAuthConfig()
  const origin = typeof request.headers.origin === 'string' ? request.headers.origin : undefined
  if (!config.configured || !isAllowedOrgmasterRequestOrigin(origin, config.config)) throw new EmployeeAuthoritySwitchError('ENTITLEMENT_AUTHORITY_SWITCH_FORBIDDEN')
}

function employeeAuthoritySwitchRequest(body: Record<string, unknown>): EmployeeAuthoritySwitchRequest {
  return {
    applicationId: body.applicationId === 'ai-pdm' ? 'ai-pdm' : String(body.applicationId ?? '') as 'ai-pdm',
    employeeId: String(body.employeeId ?? ''),
    toAuthoritySource: body.toAuthoritySource === 'legacy_authority' ? 'legacy_authority' : body.toAuthoritySource === 'orgmaster_authority' ? 'orgmaster_authority' : String(body.toAuthoritySource ?? '') as 'legacy_authority',
    expectedAuthorityVersion: Number(body.expectedAuthorityVersion),
...
async function handle(request: IncomingMessage, response: ServerResponse, root: string, devEnabled: boolean, accountEnrollmentEnabled: boolean) {
  ...
  if (path === '/employee-authority-switch' && request.method === 'POST') {
    requireGovernanceMutationOrigin(request)
    const body = await parseBody(request)
    const receipt = await switchEmployeeEntitlementAuthority(root, actor, employeeAuthoritySwitchRequest(body))
    sendJson(response, 200, receipt, current.revision)
    return
  }
