...
test('S1B-21 OrgMaster v3 direct-run profile preserves staging boundary', () => {
  ...
  assert.equal(profile.environment.fixedValues.ORGMASTER_GOOGLE_DIRECTORY_DELEGATED_SUBJECT, 'jedchang0308@jenfu.com.tw')
...
test('OrgMaster owner prepare accepts only the exact DEV-014 producer-contract remediation authority', () => {
  const priorPlainEnvironment = Object.fromEntries(profile.environment.requiredPlainEnvironmentNames
    .filter((name) => !Object.hasOwn(profile.environment.fixedValues, name) && !Object.hasOwn(profile.environment.controlledValues, name))
    .map((name) => [name, 'fixture-public-value']))
  ...
  fixture.intent.baselineIntentRef = ref('baseline-release-intent')
  fixture.values.authorization = { ...fixture.values.authorization, schemaVersion: 'orgmaster.routine-release-authorization.v1', authorizationBasis: 'OPERATOR_INVOKED_DEPLOY_PRODUCTION', devId: 'DEV-014', slice: '014-PRODUCER-CONTRACT', remediation, baselineIntentRef: fixture.intent.baselineIntentRef }
  fixture.values.readiness = { ...fixture.values.readiness, schemaVersion: 'orgmaster.routine-release-readiness.v1', devId: 'DEV-014', slice: '014-PRODUCER-CONTRACT', remediation, baselineIntentRef: fixture.intent.baselineIntentRef }
  assert.equal(assertPreparePrerequisites({ ...fixture, profile }).controlledEnvironmentAuthority.releaseMode, 'DEV014_PRODUCER_CONTRACT_REMEDIATION')
  fixture.values.readiness.remediation = { ...remediation, consumerApplicationId: 'ai-pdm' }
  assert.throws(() => assertPreparePrerequisites({ ...fixture, profile }), /CONTROLLED_ENVIRONMENT_AUTHORITY_INVALID/u)
})

test('OrgMaster owner prepare accepts only the exact DEV-014 application-registration remediation authority', () => {
  const priorPlainEnvironment = Object.fromEntries(profile.environment.requiredPlainEnvironmentNames
    .filter((name) => !Object.hasOwn(profile.environment.fixedValues, name) && !Object.hasOwn(profile.environment.controlledValues, name))
    .map((name) => [name, 'fixture-public-value']))
  const runtimeConfig = buildRuntimeConfig(profile, { plainEnvironment: resolvePlainEnvironment(profile, priorPlainEnvironment), secretVersions: Object.fromEntries(profile.environment.requiredSecretNames.map((name) => [name, '1'])) })
  ...
  fixture.intent.baselineIntentRef = ref('baseline-release-intent')
  fixture.values.authorization = { ...fixture.values.authorization, schemaVersion: 'orgmaster.routine-release-authorization.v1', authorizationBasis: 'OPERATOR_INVOKED_DEPLOY_PRODUCTION', devId: 'DEV-014', slice: '014-APPLICATION-REGISTRATION', remediation, baselineIntentRef: fixture.intent.baselineIntentRef }
  fixture.values.readiness = { ...fixture.values.readiness, schemaVersion: 'orgmaster.routine-release-readiness.v1', devId: 'DEV-014', slice: '014-APPLICATION-REGISTRATION', remediation, baselineIntentRef: fixture.intent.baselineIntentRef }
  assert.equal(assertPreparePrerequisites({ ...fixture, profile }).controlledEnvironmentAuthority.releaseMode, 'DEV014_APPLICATION_REGISTRATION_REMEDIATION')
  fixture.values.readiness.remediation = { ...remediation, requiredApplications: ['orgmaster', 'platform'] }
  assert.throws(() => assertPreparePrerequisites({ ...fixture, profile }), /CONTROLLED_ENVIRONMENT_AUTHORITY_INVALID/u)
})

test('OrgMaster owner prepare accepts only the bounded DEV-014 login-fixture correction authority', () => {
  const priorPlainEnvironment = Object.fromEntries(profile.environment.requiredPlainEnvironmentNames
    .filter((name) => !Object.hasOwn(profile.environment.fixedValues, name) && !Object.hasOwn(profile.environment.controlledValues, name))
    .map((name) => [name, 'fixture-public-value']))
  const runtimeConfig = buildRuntimeConfig(profile, { plainEnvironment: resolvePlainEnvironment(profile, priorPlainEnvironment), secretVersions: Object.fromEntries(profile.environment.requiredSecretNames.map((name) => [name, '1'])) })
  ...
  fixture.intent.baselineIntentRef = ref('baseline-release-intent')
  fixture.values.authorization = { ...fixture.values.authorization, schemaVersion: 'orgmaster.routine-release-authorization.v1', authorizationBasis: 'OPERATOR_INVOKED_DEPLOY_PRODUCTION', devId: 'DEV-014', slice: '014-LOGIN-FIXTURE-CORRECTION', correction, baselineIntentRef: fixture.intent.baselineIntentRef }
  fixture.values.readiness = { ...fixture.values.readiness, schemaVersion: 'orgmaster.routine-release-readiness.v1', devId: 'DEV-014', slice: '014-LOGIN-FIXTURE-CORRECTION', correction, baselineIntentRef: fixture.intent.baselineIntentRef }
  assert.equal(assertPreparePrerequisites({ ...fixture, profile }).controlledEnvironmentAuthority.releaseMode, 'DEV014_LOGIN_FIXTURE_CORRECTION')
  fixture.values.readiness.correction = { ...correction, applicationId: 'platform' }
  assert.throws(() => assertPreparePrerequisites({ ...fixture, profile }), /CONTROLLED_ENVIRONMENT_AUTHORITY_INVALID/u)
})
...
test('OrgMaster owner prepare accepts only the exact DEV-057 writer-fence remediation authority', () => {
  ...
  fixture.intent.baselineIntentRef = ref('baseline-release-intent')
  fixture.values.authorization = { ...fixture.values.authorization, schemaVersion: 'orgmaster.routine-release-authorization.v1', authorizationBasis: 'OPERATOR_INVOKED_DEPLOY_PRODUCTION', devId: 'DEV-057', slice: '057-WRITER-FENCE', remediation, baselineIntentRef: fixture.intent.baselineIntentRef }
  fixture.values.readiness = { ...fixture.values.readiness, schemaVersion: 'orgmaster.routine-release-readiness.v1', devId: 'DEV-057', slice: '057-WRITER-FENCE', remediation, baselineIntentRef: fixture.intent.baselineIntentRef }
  assert.equal(assertPreparePrerequisites({ ...fixture, profile }).controlledEnvironmentAuthority.releaseMode, 'DEV057_WRITER_FENCE_REMEDIATION')
...
test('OrgMaster owner prepare accepts only the exact DEV-057 principal contract authority', () => {
  ...
  const remediation = {
    ...
    contractViews: ['orgmaster_contract.v_principal_alias_history_v1', 'orgmaster_contract.v_ai_pdm_principal_effective_grants_v2', 'orgmaster_contract.v_orgmaster_session_principals_v2'],
    applicationId: 'ai-pdm',
  }
  fixture.intent.baselineIntentRef = ref('baseline-release-intent')
...
test('OrgMaster exact 001-027 production migration bytes', () => {
  ...
  assert.equal(bundle.bundle.entries[15].version, 'dev014-orgmaster-016')
  assert.equal(bundle.bundle.entries[16].version, 'dev014-orgmaster-017')
  assert.equal(bundle.bundle.entries[17].version, 'dev014-orgmaster-018')
  assert.equal(bundle.bundle.entries[18].version, 'dev014-orgmaster-019')
  assert.equal(bundle.bundle.entries[19].version, 'dev014-orgmaster-020')
  assert.equal(bundle.bundle.entries[20].version, 'dev057-orgmaster-021')
  assert.equal(bundle.bundle.entries[21].version, 'dev014-orgmaster-022')
  assert.equal(bundle.bundle.entries[22].version, 'dev014-orgmaster-023')
