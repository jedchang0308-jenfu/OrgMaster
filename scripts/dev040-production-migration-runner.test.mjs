...
test('DEV-014 runner entry count is derived from the controlled production profile', () => {
  ...
  assert.equal(profile.migrations.entries.at(-1).version, 'dev057-orgmaster-021')
})
...
test('OrgMaster runner validates the ten-row baseline and appends only 011-021', async () => {
  const input = fixture()
  ...
  assert.equal(planMigration(input.bundle, ledger).length, 11)
  const statements = []
  const database = {
  ...
  assert.equal(receipt.applied, 11)
  assert.equal(ledger.length, 21)
  assert.equal(statements.filter((value) => value === 'BEGIN').length, 11)
...
test('schema runner entrypoint migrates and publishes without reading or importing business data', async () => {
  ...
  assert.equal(statements.filter((sql) => sql.startsWith('INSERT INTO')).length, 11)
