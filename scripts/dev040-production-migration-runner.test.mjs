...
test('DEV-014 runner entry count is derived from the controlled production profile', () => {
  ...
  assert.equal(profile.migrations.entries.at(-1).version, 'dev057-orgmaster-025')
})
...
test('OrgMaster runner validates the ten-row baseline and appends only 011-025', async () => {
  const input = fixture()
  ...
  assert.equal(planMigration(input.bundle, ledger).length, 15)
  const statements = []
  const database = {
  ...
  assert.equal(receipt.applied, 15)
  assert.equal(ledger.length, 25)
  assert.equal(statements.filter((value) => value === 'BEGIN').length, 15)
...
test('schema runner entrypoint migrates and publishes without reading or importing business data', async () => {
  ...
  assert.equal(statements.filter((sql) => sql.startsWith('INSERT INTO')).length, 15)
