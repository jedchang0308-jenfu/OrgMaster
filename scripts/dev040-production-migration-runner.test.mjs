...
test('DEV-014 runner entry count is derived from the controlled production profile', () => {
  ...
  assert.equal(profile.migrations.entries.at(-1).version, 'dev014-orgmaster-020')
})
...
test('OrgMaster runner validates the ten-row baseline and appends only 011-020', async () => {
  const input = fixture()
  ...
  assert.equal(planMigration(input.bundle, ledger).length, 10)
  const statements = []
  const database = {
  ...
  assert.equal(receipt.applied, 10)
  assert.equal(ledger.length, 20)
  assert.equal(statements.filter((value) => value === 'BEGIN').length, 10)
...
test('schema runner entrypoint migrates and publishes without reading or importing business data', async () => {
  ...
  assert.equal(statements.filter((sql) => sql.startsWith('INSERT INTO')).length, 10)
