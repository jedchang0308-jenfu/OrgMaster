#!/usr/bin/env node

import assert from 'node:assert/strict'
import crypto from 'node:crypto'
import fs from 'node:fs'

const path = 'db/migrations/021_dev057_identity_grant_writer_fence.sql'
const sql = fs.readFileSync(path, 'utf8')
const statements = sql.replace(/^--.*$/gmu, '')
const hash = crypto.createHash('sha256').update(sql.replace(/\r\n/gu, '\n')).digest('hex')
const profile = JSON.parse(fs.readFileSync('config/release/dev040-orgmaster-independent-production-v3.json', 'utf8'))
const entry = profile.migrations.entries.find(({ version }) => version === 'dev057-orgmaster-021')

assert.match(sql, /^-- DB-CHANGE\n-- owner: orgmaster\n-- schemas: orgmaster_core, orgmaster_contract\n/u)
assert.match(sql, /SELECT admission_enabled[\s\S]*?managed_identity_admission_authority[\s\S]*?FOR UPDATE[\s\S]*?SELECT authority_version[\s\S]*?persistence_authority[\s\S]*?FOR UPDATE/u)
assert.match(sql, /CREATE OR REPLACE FUNCTION orgmaster_core\.write_active_persistence_artifacts_with_identity_fence_v1/u)
assert.match(sql, /principal_identity_reservations/u)
assert.match(sql, /PRINCIPAL_IDENTITY_RESERVATION_CONFLICT/u)
assert.match(sql, /ACTIVE_PRINCIPAL_MAPPING_AMBIGUOUS/u)
assert.match(sql, /CREATE OR REPLACE VIEW orgmaster_contract\.v_active_principal_mappings_v1/u)
assert.match(sql, /CREATE OR REPLACE VIEW orgmaster_contract\.v_portal_app_visibility_v1/u)
assert.deepEqual(entry, { order: 21, version: 'dev057-orgmaster-021', path, sourceSha256: hash, appliedSha256: 'c8d9b2aa02988a6b0094795b392532548f3bb7e7b51112e752355fd616334a22' })
assert.doesNotMatch(statements, /\b(?:CREATE|ALTER|DROP)\s+(?:TABLE|SCHEMA|SEQUENCE)\b/iu)
assert.doesNotMatch(statements, /\bDROP\s+(?:FUNCTION|VIEW)\b/iu)
assert.doesNotMatch(statements, /^\s*(?:GRANT|REVOKE)\b/imu)

process.stdout.write(`${JSON.stringify({ status: 'PASS', contract: 'DEV-057', migration: path, sourceSha256: hash, appliedSha256: entry.appliedSha256, productionWrites: false })}\n`)
