...
export function createReleasePlan(request, profile = loadProfile()) {
  ...
  if (request.operation === 'candidate') {
    if (!H40.test(request.sourceRevision ?? '') || !H40.test(request.sourceTree ?? '') || !IMAGE.test(request.artifactDigest ?? '') || !request.artifactDigest.startsWith(`${profile.artifact.uri}@sha256:`) || request.ssoHandoffMode !== 'on' || request.publicBaseUrl !== exactRunAppOrigin(request.publicBaseUrl, profile.target.serviceName, profile.target.region, true) || request.brokerOrigin !== exactRunAppOrigin(request.brokerOrigin, 'jenfu-platform-stg', profile.target.region, true) || !String(request.previousRevision ?? '').startsWith(`${profile.target.serviceName}-`) || typeof request.beforeProviderEtag !== 'string' || request.beforeProviderEtag.trim().length < 4 || request.platformManifestSha256 !== profile.platformManifest.sha256 || request.canonicalContractSha256 !== profile.canonicalContract.sha256) fail('DEV013_ORGMASTER_CANDIDATE_INVALID')
    ...
    candidate = { sourceRevision: request.sourceRevision, sourceTree: request.sourceTree, artifactDigest: request.artifactDigest, secretReferences: floor.secretReferences, handoffMode: 'on', publicBaseUrl: request.publicBaseUrl, brokerOrigin: request.brokerOrigin, callback: `${request.publicBaseUrl}/api/auth/jenfu-sso/callback`, previousRevision: request.previousRevision, beforeEtag: request.beforeProviderEtag }
  } else {
