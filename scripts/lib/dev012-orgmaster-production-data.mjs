...
export function buildFirstPrincipalBootstrap({ releaseId, sourceRevision, employeeIds, input, identityEvidence, observedAt }) {
  ...
  if (identityEvidence?.schemaVersion !== 'jenfu.dev012.firebase-identity-readback.v1' || identityEvidence.status !== 'PASS' || identityEvidence.releaseAuthority !== true || identityEvidence.projectId !== 'jenfu-platform-prod' || identityEvidence.uid !== input.firebaseUid || identityEvidence.emailSha256 !== emailSha256 || identityEvidence.mfaEnrolled !== false || identityEvidence.assuranceLevel !== 'aal1' || identityEvidence.totpProviderState !== 'DISABLED' || !H64.test(input.identityEvidenceRef?.sha256 ?? '')) fail('FIRST_PRINCIPAL_IDENTITY_EVIDENCE_INVALID')
