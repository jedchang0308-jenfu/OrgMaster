...
export function assertDev057PrincipalContractAppend(before, after) {
  const staticInputs = ({ sourceRevision, manifestSha256, entries, ...inputs }) => inputs
  const migrationInputs = ({ sourceRevision, manifestSha256, ...inputs }) => inputs
  if (!same(staticInputs(before), staticInputs(after)) || before.baselineCount !== 10 || before.entries?.length !== 21 || after.entries?.length !== 26) fail('DEV057_PRINCIPAL_CONTRACT_APPEND_INVALID')
  ...
  const expected = [
    ...
    ['dev057-orgmaster-026', 'db/migrations/026_dev057_session_principal_policy_path.sql', '8c212f32da3350f0e81b6fd0b9c19c19abbea793bda789ff1d4fa3dc60538322', 'e618dda50720dff23a4bd1aeb9a463cf323ff924637407f89ad75a8f26ff9fde'],
  ]
  const appended = after.entries.slice(before.entries.length)
