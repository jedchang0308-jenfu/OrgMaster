...
export function assertDev040ReleaseIntent(value, profile) {
  ...
  if (value?.baselineIntentRef) {
    expected.push('baselineIntentRef'); expected.sort()
    const ref = value.baselineIntentRef
    if (!ref.uri?.startsWith(`gs://${profile.artifact.releaseBucket}/receipts/`) || ref.uri.includes('..') || !H64.test(ref.sha256 ?? '') || Object.keys(ref).sort().join(',') !== 'sha256,uri') fail('RELEASE_INTENT_REF_INVALID', 'baselineIntentRef')
  }
...
export function assertDev040V3Profile(profile, n1c) {
  ...
  if (fixed.ORGMASTER_PERSISTENCE_MODE !== 'cloud-sql' || fixed.ORGMASTER_PUBLIC_BASE_URL !== target.canonicalOrigin || fixed.ORGMASTER_POSTGRES_POOL_MAX !== '6' || fixed.ORGMASTER_POSTGRES_QUERY_TIMEOUT_MS !== '35000'
    ...
    || fixed.ORGMASTER_JENFU_SSO_HANDOFF_MODE !== 'off'
