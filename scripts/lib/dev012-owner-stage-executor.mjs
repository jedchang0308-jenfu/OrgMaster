...
function assertControlledEnvironmentAuthority({ intent, profile, values, runtime }) {
  ...
  if (!Object.entries(rules).some(([name, rule]) => controlledEnvironment[name] !== rule.defaultValue)) return
  ...
  if (values.authorization?.schemaVersion !== 'jenfu.dev013.l4-owner-transition-authorization.v1' || values.authorization.authorizationBasis !== 'OPERATOR_INVOKED_DEV013_L4'
    ...
    || !Object.hasOwn(rules, transition?.field) || transition.to !== controlledEnvironment[transition.field] || !rules[transition.field].allowedValues.includes(transition.from) || !rules[transition.field].allowedValues.includes(transition.to) || transition.from === transition.to
    || !['activate', 'advance', 'rollback'].includes(transition.action)
