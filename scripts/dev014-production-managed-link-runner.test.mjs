...
function database({ replay = false } = {}) {
  const calls = []
  const query = async (sql, params) => {
    calls.push({ sql, params })
    if (sql.includes('read_employee_managed_identity_v1')) return { rows: [{ employee_id: TARGET.employeeId, employee_status: 'active', employee_number: TARGET.employeeNumber, identity_state: replay ? 'directory_linked_pending_auth' : 'not_linked', registry_revision: '1', admission_enabled: true }] }
