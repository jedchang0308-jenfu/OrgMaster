...
test('N2-ORG-06 browser rehearsal builds synthetic governance state without local business data', () => {
  ...
  assert.match(browserSource, /const orgMirror = path\.join\(taskRoot, 'orgmaster'\)/u)
  assert.match(browserSource, /orgmaster-workspace\.v1\.json/u)
  assert.match(browserSource, /orgmaster-versions/u)
  assert.match(browserSource, /name: '選擇測試角色'/u)
  ...
  assert.match(browserSource, /suggested name did not become actionable after hydration/u)
  assert.match(browserSource, /confirmed name was not applied/u)
