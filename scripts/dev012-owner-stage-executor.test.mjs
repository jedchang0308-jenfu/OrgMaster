...
function recordedHarness() {
  ...
  const transport = {
    ...
    async prepareCandidateEndpoint() { service = { ...service, defaultUriDisabled: false, ingress: 'INGRESS_TRAFFIC_INTERNAL_ONLY' }; return structuredClone(service) },
    async enableCanonicalIngress({ expectedRevision }) { assert.equal(effectiveRevision(service), expectedRevision); service = { ...service, ingress: 'INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER' }; return structuredClone(service) },
