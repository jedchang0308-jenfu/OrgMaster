...
import { AuthApiError, exchangeFirebaseToken, getAuthMode, getCurrentSession, installDevelopmentApiIdentityHeaders, logoutCurrentSession, type AuthMode, type AuthSessionView } from './authApiClient'
...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    let active = true
    installDevelopmentApiIdentityHeaders()
  ...
  if (state.kind === 'authenticated' || state.kind === 'logout-processing' || state.kind === 'logout-failed') {
    return <>
      {children}
      <aside className="auth-session-bar" aria-label="登入狀態">
        <span>員工 {state.session.user.employeeId}</span>
        {state.kind === 'logout-failed' && <span role="alert">登出失敗，工作階段仍有效。</span>}
        <button type="button" onClick={() => { void logout() }} disabled={state.kind === 'logout-processing'}>
          {state.kind === 'logout-processing' ? '登出處理中…' : state.kind === 'logout-failed' ? '重試登出' : '登出'}
        </button>
      </aside>
    </>
  }
