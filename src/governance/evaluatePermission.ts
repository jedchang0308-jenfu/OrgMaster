...
function evaluateV1(document: GovernanceDocumentV1, request: PermissionEvaluationRequestV1, options: Options, now: string, version: GovernancePolicyVersionV1 | null): PermissionEvaluationResultV1 {
  ...
  const policy = version.policy; const permission = policy.permissions.find((value) => value.applicationId === request.applicationId && value.code === request.permissionCode); if (!permission || permission.status !== 'active') return { ...result, reason: 'PERMISSION_UNKNOWN' }
  ...
  if (!request.scope || request.scope.kind !== 'global' && !request.scope.value.trim()) return { ...result, reason: 'SCOPE_REQUIRED', principalId: link.principalId }
  const activeRoleIds = new Set(policy.applicationRoles.filter((role) => role.applicationId === request.applicationId && role.status === 'active').map((role) => role.id)); const roleIds = new Set(policy.roleAssignments.filter((assignment) => activeRoleIds.has(assignment.roleId) && assignment.employeeId === link.employeeId && assignment.status === 'active' && isActiveAt(assignment.status, assignment.validFrom, assignment.validTo, now) && scopeMatches(assignment.scope, request.scope)).map((assignment) => assignment.roleId)); const grants = policy.rolePermissionGrants.filter((grant) => roleIds.has(grant.roleId) && grant.permissionId === permission.id)
  if (grants.some((grant) => grant.effect === 'deny')) return { ...result, reason: 'EXPLICIT_DENY', principalId: link.principalId, matchedRoleIds: [...roleIds] }
...
export function evaluatePermission(document: GovernanceDocumentV1 | GovernanceDocumentV2 | GovernanceDocumentV3, request: PermissionEvaluationRequestV1, options: Options = {}): PermissionEvaluationResultV1 {
  ...
  if (document.schemaVersion === 2) {
    if ((active as any)?.kind === 'legacy-policy-v1') return evaluateV1(document as unknown as GovernanceDocumentV1, request, options, now, active as GovernancePolicyVersionV1)
    return evaluateV2(document, request, options, now, (active as any)?.kind === 'assignment-governance-v2' ? active as GovernanceAssignmentVersionV2 : null)
  }
