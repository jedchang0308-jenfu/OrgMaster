...
describe('V1 to V2 governance migration', () => {
  it('keeps V1 bytes untouched, maps supported assignments, and records unresolved legacy data', () => {
    ...
    legacy.draft.roleAssignments.push({ id: 'assignment-rd', employeeId: 'employee-1', roleId: 'role-rd', scope: { kind: 'workspace', value: 'workspace-1' }, status: 'active', validFrom: '2026-08-27T00:00:00.000Z', validTo: null })
    ...
    expect(migrated.draft.roleAssignments[0]).toMatchObject({ applicationId: 'ai-pdm', roleId: 'role-rd', catalogVersion: readAiPdmRoleCatalog('valid').catalogVersion, effectState: 'not-synchronized' })
