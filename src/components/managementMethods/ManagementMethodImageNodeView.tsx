...
export function ManagementMethodImageNodeView({ node, editor, extension, getPos, selected, updateAttributes, deleteNode }: ReactNodeViewProps) {
  ...
  const startResize = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    resizeRef.current = { pointerId: event.pointerId, startX: event.clientX, startWidth, currentWidth: startWidth }
    ...
    const finish = (finishEvent: PointerEvent) => {
      ...
      if (state.currentWidth !== width) updateAttributes({ width: state.currentWidth })
    }
  ...
  return <NodeViewWrapper as="figure" className={`management-method-editor-image${selected ? ' is-selected' : ''}`}>
    <div ref={imageCanvasRef} className="management-method-editor-image__canvas" style={displayedWidth ? { width: `${displayedWidth}px` } : undefined}>
      ...
      {selected ? <button type="button" className="management-method-editor-image__resize-handle" role="slider" aria-label="調整圖片寬度" aria-valuemin={MANAGEMENT_METHOD_IMAGE_MIN_WIDTH} aria-valuemax={MANAGEMENT_METHOD_IMAGE_MAX_WIDTH} aria-valuenow={accessibleWidth} aria-valuetext={`${accessibleWidth} 像素`} onPointerDown={startResize} onClick={(event) => { event.preventDefault(); event.stopPropagation() }} onKeyDown={resizeWithKeyboard}><span aria-hidden="true" /></button> : null}
    </div>
    ...
    {selected ? <div className="management-method-editor-image__controls" aria-label="圖片設定" onPointerDown={(event) => event.stopPropagation()}>
      ...
      <div className="management-method-editor-image__actions">
        <output aria-live="polite">{resizingWidth ? `${resizingWidth} px` : width ? `${width} px` : '原始尺寸'}</output>
