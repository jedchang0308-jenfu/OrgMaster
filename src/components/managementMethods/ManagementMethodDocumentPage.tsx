...
interface Props { methodId: string; initialView: 'draft' | 'readable'; state: OrgDirectoryState; onClose: () => void }
export function ManagementMethodDocumentPage({ methodId, initialView, state, onClose }: Props) {
  const [method, setMethod] = useState<ManagementMethodV1 | null>(null); const [session, setSession] = useState<ManagementMethodSessionV1 | null>(null); const [view, setView] = useState(initialView); const [editing, setEditing] = useState(false); const [allowed, setAllowed] = useState(() => canMutateManagementMethods()); const [pendingBody, setPendingBody] = useState<EditorDocumentV1 | null>(null); const [titleDraft, setTitleDraft] = useState(''); const [ownerDraft, setOwnerDraft] = useState(''); const [error, setError] = useState(''); const [notice, setNotice] = useState(''); const [dutyOpen, setDutyOpen] = useState(false)
  ...
  const uploadImages = async (files: File[]) => {
    ...
    for (const file of files.slice(0, 10)) {
      try {
        ...
        const nextBody = JSON.parse(JSON.stringify(pendingBody ?? method.workingDraft.body)) as EditorDocumentV1
        nextBody.content.push({ type: 'methodImage', attrs: { mediaId: result.media.id, altText: result.media.altText, caption: '' } })
  ...
  return <div className="management-method-document-page"><header className="management-method-document-page__header"><button type="button" onClick={onClose}><ArrowLeft size={17} />管理辦法</button><div><BookOpen size={18} /><span>{method.code}</span></div><div className="management-method-document-page__actions">{!editing && canEdit && <button type="button" className="button-primary" onClick={() => { setEditing(true); setPendingBody(method.workingDraft.body) }}><Edit3 size={16} />編輯文件</button>}{!editing && <button type="button" className="button-secondary" onClick={() => setDutyOpen(true)}><Users size={16} />職掌對照</button>}</div></header><main className="management-method-document-page__main">{error && <div className="management-method-error" role="alert">{error}<button type="button" onClick={() => void load()}>重試</button></div>}<div className="management-method-document-page__identity"><span className="management-method-code">{method.code}</span><span>{status}</span>{view === 'readable' && <span>公司閱讀</span>}{!allowed && <span>唯讀</span>}</div>{editing ? <section className="management-method-edit-surface"><div className="management-method-edit-surface__meta"><label>標題<input value={titleDraft} onChange={(event) => setTitleDraft(event.target.value)} maxLength={120} /></label><label>負責人員 ID<input value={ownerDraft} onChange={(event) => setOwnerDraft(event.target.value)} placeholder="提供公司閱讀前填寫" /></label><button type="button" className="button-secondary" onClick={() => void metadata()}>保存資訊</button></div>{body && <ManagementMethodEditor body={body} onChange={setPendingBody} onFile={uploadImages} />}</section> : <><h1>{method.title}</h1><ManagementMethodReader method={method} view={view} /></>}<div className="management-method-document-page__footer">{notice && <span role="status"><Check size={14} />{notice}</span>}{editing && <div className="management-method-edit-actions"><button type="button" className="button-secondary" onClick={() => { setEditing(false); setPendingBody(null) }}>完成編輯</button><button type="button" className="button-primary" onClick={() => void save()}>保存草稿</button>{canProvide && <button type="button" className="button-primary" onClick={() => void provide()}>提供公司閱讀</button>}{method.readableSnapshot && canProvide && <button type="button" className="button-secondary" onClick={() => void restore()}><History size={15} />還原閱讀版本</button>}{method.readableSnapshot && canProvide && <button type="button" className="button-danger" onClick={() => void stop()}>停止閱讀</button>}</div>}</div></main>{dutyOpen && <ManagementMethodDutyDrawer state={state} onClose={() => setDutyOpen(false)} />}</div>
}
