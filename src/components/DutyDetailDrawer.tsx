...
const relationOrder: DutyRelationType[] = ['execute', 'review', 'collaborate', 'countersign']
...
export function DutyDetailDrawer({ duty, state, editingEnabled, onClose, onPatchDuty, onRemoveRelation, onCreateRelation, onTransfer, onDeleteDuty }: DutyDetailDrawerProps) {
  ...
  return (
    <aside className="duty-drawer" aria-label="工作執掌明細">
      ...
      <div className="duty-drawer__body">
        <label className="duty-field">名稱<input value={draftTitle} disabled={!editingEnabled} maxLength={120} onChange={(event) => setDraftTitle(event.target.value)} onBlur={commitText} /></label>
        <label className="duty-field">說明<textarea value={draftDescription} disabled={!editingEnabled} maxLength={2000} rows={4} onChange={(event) => setDraftDescription(event.target.value)} onBlur={commitText} /></label>
