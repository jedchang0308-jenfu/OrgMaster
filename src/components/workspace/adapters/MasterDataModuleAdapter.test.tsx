...
describe('MasterDataModuleAdapter', () => {
  it('keeps list and selected detail in one panel', async () => {
    ...
    expect(workspace?.children[2].classList.contains('master-data-workspace__detail')).toBe(true)
    root.unmount()
  })

  it('keeps an empty detail slot when the detail is closed', async () => {
    const host = document.createElement('div')
    const root = createRoot(host)
    await act(async () => {
  ...
  it('uses the full panel for levels without inventing a second detail surface', async () => {
    ...
    expect(host.querySelector('[data-layout="adjacent-list-detail"]')).not.toBeNull()
    expect(host.querySelector('.master-data-workspace__detail')).not.toBeNull()
    root.unmount()
  })
})
