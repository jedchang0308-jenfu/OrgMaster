...
> **2026-09-25 DEV-057 Production R2 與 026 修正（現行）**：R2 workflow 36139874430 已把正式 ledger 更新至 25（本輪 2 applied／23 replayed），候選版 smoke 的 session create 因 principal_not_active 回 403，於 activate 前安全中止；正式 OrgMaster traffic 仍 100% 在 orgmaster-prod-224778008119。已定位 migration 022 的 v2 session view 讀錯 governance policy 巢狀路徑，新增 forward-only 026 修正，保留 022／歷史 receipts；隔離 PostgreSQL 18.4 D57-01～18 18／18 PASS，owner release 聚焦驗證通過。026 尚未 Production 套用或切流；下一步在同一 DEV-057 子任務完成 source-frozen owner release、candidate smoke、readback 與跨 owner consumer 驗證。詳見 DEV-057 spec 的 Production R2 段。
