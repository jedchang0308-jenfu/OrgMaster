...
> **2026-09-26 DEV-057 精確 pair Production 結果（現行）**：protected master `9ffa1f25f951560b8d0105b653c472cc7c260e73` 的 owner 唯讀 Job 成功，私有 receipt `gs://jenfu-platform-prod-orgmaster-release/receipts/releases/DEV057-PRINCIPAL-PAIR-DIAGNOSTIC/20260926-r1.json#1790372778753938`（SHA-256 `cba192ba941b4d8e3849a39457fa7f8256c9309a49a8246529a92331674ec1db`）與 AI-PDM R4 的六個 pair hash 對齊：僅既有管理者 Firebase pair 同時有 canonical／typed／治理／reservation；其餘五組在 canonical、typed、managed registry、active governance、permanent reservation 均缺列。一筆未綁 pair 的 managed identity 不能猜作缺列之一。AI-PDM R5 私有 readback 進一步區分三個未映射 Firebase pair 與兩個已關閉登入路由的歷史 Google OAuth alias。此為來源歸屬缺口，非單一 projection SQL 錯誤；不得重跑 authority switch 或以 email／姓名猜 Employee。臨時 OrgMaster Job 已刪除，image／receipt 保留；沒有 service、traffic、schema、帳號或權限變更。後續 owner 核對、正式綁定與 L4 仍在原子任務續行。

> **2026-09-26 DEV-057 provider-pair 診斷（唯讀）**：AI-PDM DEV-121 受控 inventory 顯示五組現有 provider pair 缺 OrgMaster canonical mapping。於同一 `ORGMASTER/DEV-057#identity-grants` 增加 hash-bound、私有收據、只讀 snapshot 的 owner operator，比對 mapping／typed／managed／published governance／reservation，以釐清缺列來源。未有 owner readback 前，不註冊 inventory、不重跑 authority switch、不認列 L4。見[DEV-057 診斷契約](specs/DEV-057-identity-and-grant-contract-boundary.md#production-r3-release)。

> **2026-09-26 DEV-057 Production R3（owner producer 已發布）**：官方 `master` `48120534cbde4a06d0f3cd6d5de76171ca7e0699` 的 [owner workflow 36146949383](https://github.com/jedchang0308-jenfu/OrgMaster/actions/runs/36146949383) 十階段全數成功，terminal `RELEASED`。正式 migration ledger=26（本輪 026 applied=1、既有 25 replayed），Cloud Run `orgmaster-prod-c2a14a18803b` 已承接 100% traffic；[R3 source／migration／traffic 證據](qa/DEV-057-production-r3-release-evidence-2026-09-26.md)已核對。OrgMaster producer 發布不等於 AI-PDM v2 consumer、完整 managed identity readback、跨 assertion TTL、v1／v2 recovery 或三系統 Production L4 完成；這些仍在同一 principal-first 任務續行。下方 R2 安全中止及當時「026 未發布」為歷史快照。

> **2026-09-25 DEV-057 Production R2 與 026 修正（歷史）**：R2 workflow 36139874430 已把正式 ledger 更新至 25（本輪 2 applied／23 replayed），候選版 smoke 的 session create 因 principal_not_active 回 403，於 activate 前安全中止；正式 OrgMaster traffic 仍 100% 在 orgmaster-prod-224778008119。已定位 migration 022 的 v2 session view 讀錯 governance policy 巢狀路徑，新增 forward-only 026 修正，保留 022／歷史 receipts；隔離 PostgreSQL 18.4 D57-01～18 18／18 PASS，owner release 聚焦驗證通過。026 尚未 Production 套用或切流；下一步在同一 DEV-057 子任務完成 source-frozen owner release、candidate smoke、readback 與跨 owner consumer 驗證。詳見 DEV-057 spec 的 Production R2 段。
...
> **2026-09-24 DEV-014 existing-account normal-entry observation（現行證據）**：沿用已登入的 `jedchang0308@jenfu.com.tw` Platform session 開啟 OrgMaster production `run.app` URL；OrgMaster 未要求第二次 Google 登入，頁面顯示 `employee-shijie` 並載入受保護組織／角色治理工作台。此為既有帳號的 redacted normal-entry／single-session observation，不宣稱 Free-only bridge、authority switch、assertion-TTL、permission API、global logout、rate／race 或完整 Production L4 denominator 已 PASS。

> **2026-09-24 DEV-014 login-fixture correction continuation R11（現行證據）**：以官方 protected `master` clean source `bdd382a8f732b61a622bca696666a2aa5e77de0d` 完成 owner-native immutable runner rotation。APP_INFRA_IMAGE_ROTATION gate為 `0 add／1 update／0 destroy`，只更新既有 `orgmaster-prod-migration-runner` image；plan SHA-256=`52f7e91042dd3199b5fbf181e66de4c2c99624a87aaca22566d9127ba6b2faac`，gate addressActions SHA-256=`7b39db66c6f1742a5dfcd83f0d2676accfa4b27369b93c21c028cc0905580ff4`，foundation manifest SHA-256=`5916aac21b791c3ae25c320532c5d214350b8049ee73828c4134ec92ec385e40`，app-infra receipt=`gs://jenfu-platform-prod-orgmaster-release/receipts/releases/DEV014-INFRA-ORGMASTER-20260924-R11/app-infra.json`（SHA-256=`2969591b72175348364dd9440d4fed059472d50ef68ffa536b07b61661530ab2`），immutable runner=`asia-east1-docker.pkg.dev/jenfu-platform-prod/orgmaster-release/orgmaster-migration-runner@sha256:aab49a061c7ec127f84fa5ba6d42f085c055fee703c2382e050a9f07fdcc82b0`。官方 owner run `35902145983` 的 `prepare／build／migrate／candidate／entrypoint／verify／decision／activate／canonical／finalize` 全部成功，`orgmaster-prod`維持100%至 `orgmaster-prod-224778008119`；未改 schema、data、IAM、Secret或其他 service。其後 authority execution `orgmaster-prod-migration-runner-62p6m` 在 CAS 前 readback 缺少 `access_governance.v_active_principal_links_v1` managed-identity bridge，以 `DEV014_LOGIN_AUTHORITY_AUTH_BRIDGE_REQUIRED` 安全停止；Cloud Logging確認無 authority／receipt／outbox mutation，Job已恢復 `--bundle-ref-required` baseline。瀏覽器 task tab 已開啟 Google chooser 並顯示既有 `dev014-fp-google@jenfu.com.tw`，但目前互動層無法送出帳號選取；未輸入或保存密碼、OTP、CAPTCHA。Free-only bridge、兩筆 authority switch及Production L4仍待完成首次 managed-identity bridge後再執行。

> **2026-09-24 DEV-057 QC portability correction（現行證據）**：`scripts/qc-dev-057-contract.mjs` 現先將 migration 文字正規化為 LF，再執行契約標頭／物件／forward-only assertions；source hash 仍以同一 normalized bytes 計算，未改動 migration SQL、profile 或 applied hash。修正後 `qc:dev-057:contract` PASS，隔離 PostgreSQL 18.4 `qc:dev-057:postgres` 的 D57-01～D57-06 全部 PASS，執行 0 primary-data writes 且 client／cluster／port／temp cleanup 全部完成。此修正消除 Windows CRLF checkout 造成的 false negative，不放寬任何 production gate。

> **2026-09-23 DEV-014 operator identity fence correction（現行）**：Production Job `orgmaster-prod-migration-runner` 的既有 service identity 與 DB login 都是 `orgmaster-prod-migrator`；fixture operator 原先錯誤要求 `orgmaster-prod-runtime`，因此所有 phase 會在任何資料庫操作前以 `DEV014_LOGIN_FIXTURE_TARGET_INVALID` 安全停止。修正將 operator 的 metadata／IAM DB fence 對齊既有 migrator identity，保留 runtime service 的獨立邊界；不新增權限、schema、migration、Secret、service 或 traffic。Targeted fixture tests `10/10 PASS`，Production execution 須以修正後同 source 的 immutable runner 重新 build／rotation，舊 execution 不得重播。

> **2026-09-23 DEV-014 fixture active-revision readback correction（現行）**：Production governance v4 已發布兩筆精確 `ai-pdm / rd / workspace:current` 指派；後續 activation 因 UI 維護已刷新 active workspace canonical revision，而既有 operator 的 `readback` receipt 未回傳該必要機器綁定，第一次重播以 `DEV014_LOGIN_FIXTURE_WORKSPACE_REVISION_CONFLICT` 在寫入前安全停止。修正只讓既有 `readback` phase 透過受控 persistence routine 回傳 `workspaceVersionId／workspaceRevision`，不新增 phase、schema、IAM、Secret、service、人工 SQL 或資料 mutation；取得 fresh revision 後沿用原 `activate → link → readback` 及 Job baseline restore。
> **2026-09-23 DEV-014 fixture authority projection correction（現行）**：兩筆新 fixture 的 `ai-pdm / rd / workspace:current` assignment 已存在且 active，但 authority 仍為 `legacy_authority:1`，故 effective projection與Portal保持空集合。新增 source-controlled `dev014-production-login-authority-switch-runner.mjs`，只接受兩個固定 Employee ID、既有 `access_governance.switch_employee_entitlement_authority_v1`、`legacy_authority:1 → orgmaster_authority:2`及`rd` role；兩筆切換在同一serializable transaction完成，receipt／outbox成對、replay不增加版本，partial state／scope／identity／role drift均在CAS前停止。第二輪審查將operator source與governance version／hash／persistence source分離綁定，durable receipt只含可由committed DB rows重建的不變事實，使APPLIED／REPLAY產生相同bytes並可補救首次publish失敗。Dockerfile、operator target、Platform plan已同步升至v4；本地authority 12／12與fixture 10／10 PASS。Production R10 preflight 另補上對 `access_governance.v_active_principal_links_v1` 的同源 bridge readback；若 Free-only 帳號尚未完成首次登入，會在 CAS 前以 `DEV014_LOGIN_AUTHORITY_AUTH_BRIDGE_REQUIRED` 停止，不再把資料庫 `23503` 暴露為操作結果。正式 authority switch 與 browser L4 仍待 bridge 完成後執行。

> **2026-09-23 DEV-014 managed-login bridge boundary correction（現行）**：Free-only fixture 只有已發布的 `ai-pdm / rd / workspace:current` assignment，沒有 OrgMaster application role；原 bridge 以 OrgMaster role 作為唯一登入資格，導致 Platform 已登入卻在 `/api/sso/authorize` 以 `application-not-available` 中止。現將「首次建立 managed identity link」與「OrgMaster 應用授權」分離：bridge 只接受已發布且 active 的 OrgMaster 或 AI-PDM assignment，完成後仍由既有 OrgMaster permission gate 控制 OrgMaster UI，不能藉 bridge 取得 OrgMaster 權限。`?bridge=1` 是唯一明示 bootstrap 入口，正常 SSO handoff 不變；targeted managed-identity tests `37/37` 與 client/server build PASS，尚待 fresh owner release、bridge readback 與後續 authority／L4 驗證。
...
> **2026-09-22 DEV-014 zero-paid-seat completion gate v2（現行）**：來源為`Jenfu-Platform / DEV-014 / zero-paid-seat fixture gate`。人類已明確決定不購買、不回收、不重配任何Workspace付費席次；唯讀重用`employee-shijie / jedchang0308@jenfu.com.tw`既有active證據，只新建`dev014-fp-google`與`dev014-fp-number`兩個Cloud Identity Free-only／無Gmail帳號及各自標記的OrgMaster disposable Employee。Google Admin readback為Cloud Identity Free尚有50席、Workspace Business Standard可用0席，且`OrgMaster`機構單位已覆寫Workspace自動授權為關閉。Platform plan已升為v2、共同finalizer升為`jenfu.dev014.login-production-completion.v2`；不得修改其他既有帳號、Employee、binding、assignment或permission，完成後只暫停這兩個新帳號、retire兩筆標記fixture並撤銷fixture sessions，保留契約要求的binding／audit rows。
...
- ◐ DEV-053 [修復點] [P0] [Production Migration + Admission Complete／Production L4 Pending] Managed identity invalidation application registration補正
  ...
  - 發布：exact 001–016→017與source-matched runner已套用及重播；receipt固定17 rows，後續ordinary release只允許完整001–017 unchanged。
...
- ✓ DEV-051 [開發點] [Production authority switch + AI-PDM L4 Complete] [P0] [RD／QA-QC／Production Complete] AI-PDM 單一員工權限來源切換控制面
  ...
  - Production結果：一次性source-bound operator已完成`employee-shijie / ai-pdm`切換、replay、outbox與readback，final authority=`orgmaster_authority:6`；Platform／AI-PDM L4、權限API、global logout與logout後401均PASS。
...
- ◐ DEV-050 [交付點] [Production Released／Canonical-first Correction Released／Workspace L4 Partial] [RD Implementation Complete／Architecture Finalized R2＋Release Amendment／Local QA-QC Passed] 員工編號或公司 Email 單一 Google 身分登入
  ...
  - 下一步：owner release、migration、DWD、admission、managed link與canonical-first correction皆已完成；Workspace工號callback及AI-PDM `PDM-W-E`亦已關閉，只補Free-only／無Gmail fixture、negative／rate／race及AI-PDM兩個Free target cells。
  - 阻塞 / 恢復條件：OrgMaster current revision=`orgmaster-prod-fda3dbbe8347`且Workspace Google-first／工號起手→AI-PDM、reload、權限與global logout均有current evidence；仍缺既有核准的Free-only／無Gmail fixture與完整negative／rate／race矩陣，不重做未漂移migration、link或admission。
...
- ✓ DEV-048 [開發點] [L3 managed staging complete] [P0] [OWNER_READY_FOR_L3_BROWSER／releaseAuthority=false] DEV-013 `013-S4-L3-ORGMASTER-ENV` OrgMaster managed staging runtime與owner release package
  ...
  - 結果：上述owner package已實際在`jenfu-platform-nonprod`執行；final source `9b8a6a1846ff53993cdc61dda30caff24f2c7e90`、revision `orgmaster-stg-00005-wum`、mode=`on`、100% revision-pinned traffic並產生`OWNER_READY_FOR_L3_BROWSER`。
  - 下一步：無；nonprod L3交付已完成，後續只作未漂移證據重用或另行核准的修正。
  ...
  - 計入交付：是（L3 managed staging owner交付完成；releaseAuthority仍為false）。
...
- [x] exact source owner-native Production release與target-session correction已完成。
- [x] `employee-shijie / ai-pdm` authority receipt、L4 browser、global logout與觀察完成；final authority=`orgmaster_authority:6`。

`Production Complete / Authority Switch + AI-PDM L4 + Global Logout PASS`：一次性operator、replay、outbox、權限API及logout後401均已驗證；不得重跑或以人工SQL、migration Job override等旁路改寫終態。

## DEV-050：員工編號或公司 Email 單一 Google 身分登入

狀態：`Production Released / Canonical-first Correction Pending / Workspace Target Partial Evidence`
...
- Google Admin／licensing／provider write仍不在本輪範圍；正式migration／activation／deploy依DEV-040 §36與fresh exact Production L4 authorization執行。
...
- [x] 本機產品 QA／QC evidence 已產生並核對 cleanup。
- [ ] 發布OrgMaster canonical-first correction，確認new revision／100% canonical traffic，再完成Google／工號、Free、deny-path、OrgMaster target及global logout的full Production browser matrix。
...
狀態：`Production Released / DWD + Admission + Single-employee Link Complete / Canonical-first Correction Ready`
...
- 2026-09-21 DEV-014 re-entry 已修正正式 Directory credential：canonical five-key config、runtime ADC→專用 signer IAM Credentials `signJwt`→read-only OAuth token，並加入只擁有 signer 與 signer-level Token Creator 的 app-owned Terraform；不建立或保存 service-account key。Google Admin read-only核對固定customer ID=`C015t4buc`及有效超級管理員delegated subject=`jedchang0308@jenfu.com.tw`；signer、唯一read-only DWD grant、runtime config與base deploy均已完成provider readback。
- 2026-09-21 DEV-049 production admission operator已完成own-bucket evidence amendment：Production fail-seeking execution `orgmaster-prod-migration-runner-ct65x`在DB mutation前以`MIGRATION_GCS_METADATA_FAILED`停止，證明migrator不可跨owner bucket讀receipt，Job已回復baseline。現改由受控operator先驗origin raw SHA，再以create-only建立OrgMaster own-bucket byte-identical mirror；operation固定origin＋mirror＋source／artifact／SHA，runner只讀own bucket並重驗完整conformance，DB evidence仍保存origin ref＋SHA。禁止新增跨bucket IAM、人工SQL、新Job、runtime DDL與sibling core access；targeted admission tests 5／5 PASS，待fresh immutable runner image及Production apply／replay。
- 2026-09-21 DEV-014 protected release補強：DWD Terraform加入source／foundation／operator provenance；machine plan gate固定四地址configuration、runtime `prior_state` exact enabled readback與三個managed change地址，避免把plan-time已完成的data read誤當`resource_changes`。owner finalize發布`jenfu.dev014.consumer-conformance.v1`，admission runner在DB前驗三份raw-object SHA、schema、source、artifact及content hash。focused gate目前17／17 PASS；production apply須使用merged clean source及provider-readback saved plan。
- P0／P1 設計待決項=0；一般產品、managed-link operator apply＋replay與base release已完成。2026-09-22 canonical-first correction已通過local gate；Platform correction已發布並取得Workspace→AI-PDM partial evidence，OrgMaster correction仍待owner dispatch。
