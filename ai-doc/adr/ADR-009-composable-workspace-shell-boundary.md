...
狀態：Accepted / DEV-039～042 Baseline Complete / DEV-046 Amendment Accepted and RD Implementation Ready（2026-09-04）

> **2026-09-04 DEV-046 unified list-detail amendment（現行）**：八個指定功能 `employees／positions／departments／levels／duties／processes／management-methods／role-risks` 的最外層surface統一為左清單＋可調separator＋永遠存在的右detail frame。Level先使用空detail slot；Process把既有canvas、node editor與Duty bridge保留在右detail內；Duty三模式及Role Risk只拆slot，不泛化domain內容。共用權威沿用既有`WorkspaceListDetailSurface`，不得建立第二個`WorkbenchShell`、schema-driven mega component或逐module selected state。清單preferred width由verified principal＋module的private account preference擁有，OrganizationDocument、workspace version、URL及browser-local layout都不保存；container clamp只產生effective width，不回寫帳號。Row click／Arrow／Escape與focus由單一interaction contract處理，domain dirty guard優先。Relation extension仍沿用DEV-041的typed payload、single session、`resolveRegisteredDrop()`、shared bindings與App mutation owner，不新增plugin registry、event bus或第二pair matrix。此為使用者明確核准的`Intentional replacement`，取代下方DEV-042「Level list-only／Process最外層例外／固定清單寬度／關閉detail即移除右欄」條文；Organization Canvas與Governance Single surface不在本次scope。權威工程契約、migration／API、S0～S6及QA／QC見[DEV-046](../specs/DEV-046-unified-list-detail-workbench-framework.md)。

> **2026-09-02 repository integration override（現行）**：DEV-039 workspace baseline=`164a779`，DEV-041／042與最終整合=`4e3b2ce`，merge=`c8cc16f`；目前已納入`master`。舊段落所列不可達`86510f4`、Candidate Freeze／Authorization Pending與未授權commit／merge只作整合前provenance，不再是現行gate。DEV-034／038保留domain不變量，固定UI與專用拖曳入口已被本ADR下的DEV-039／041／042 intentional replacement關閉。deploy／release未要求，另走共用release gate。

> **2026-09-02 DEV-042 single-layer workspace amendment（設計期契約／已完成）**：使用者決定移除「頂部功能 → 快速Drawer → 在工作台開啟 → panel」的第二層入口。十個既有module統一由頂部launcher直接open-or-focus唯一panel；`DrawerWorkspaceModuleId`、drawer registry／session／controller／renderer、`WorkspaceQuickDrawer`與promotion語意在同一intentional replacement中移除，不以CSS隱藏或長期feature flag保留。技術主管審查後，detail visibility只由`WorkspaceSessionState.openDetails`擁有，`WorkspaceRouteState.openDetails`與browser URL沿用既有`routeFromState／reconcileRoute`作投影，不在PanelSession建立逐panel副本；registry只以`supportsCollapsibleDetail`判斷shell收合責任，不建立三態runtime taxonomy；`WorkspaceOpenIntent`是否帶context直接決定preserve／replace，不另加`contextMode`。Process保留內建流程清單＋graph＋bridge，Level維持既有list-only，禁止為形式一致製造雙清單或空detail。`WorkspaceLayoutV1`、每類一份panel、split／tab／resize／pin、canonical `/`、DEV-041 single relation placement及各domain authority不變。本amendment已由DEV-042實作、QA／QC與repository integration完成；本文其後關於Drawer promotion的舊條文只作provenance，對現行runtime不再具拘束力。

> **2026-09-02 DEV-042 validation closure（現行）**：四個 DEV-040 governance／catalog 測試契約漂移已依 workspace scope／current catalog version 修正；management-method local-development actor precedence 已修正並補 regression。完整回歸 `174 files／716 passed／1 skipped`，targeted gate、typecheck、client／server build與production source scan通過。Fresh task-owned 5080 fixture完成 E1～E9；DEV-041四向native、zero-mutation、auto-pan與console/pageerror沿用最新aggregate覆核。唯一總表=`output/playwright/dev042/manifest.md`。DEV-042已完成Browser QA／QC並隨`4e3b2ce`／`c8cc16f`納入`master`；deploy／release未要求。
...
> **2026-09-01 candidate provenance（已取代）**：舊工作樹曾完成selective candidate freeze，但其文件commit `86510f4`在目前repository不可達。現行Git權威改為`164a779`、`4e3b2ce`及merge commit `c8cc16f`；不得依本段重新派工candidate或merge。
...
- S7現為`Relation Placement Implemented / QA-QC Passed / Repository Integrated`；DEV-039第22.6、25.6、26.8節及第26.13.1節仍固定single App owner、pure reducer、exact repo／symbol、capability、runtime migration、舊state removal allowlist、S7-0～S7-5、targeted／aggregate test、B16 executable fixture與最小 failure／lifecycle case matrix。E1四個 minimum directions、E2五案`historyEvidence`與E3兩案均已由正式QA-QC覆核；full regression為`160 files／664 tests（1 skipped）`，typecheck、build、source scan與runtime cleanup通過；repository integration由`164a779`、`4e3b2ce`與`c8cc16f`閉合。
- S7 evidence closure（2026-09-02）：第26.21.19節的五案 E2 與第26.21.17節的 E3-WARNING均已有可採用 evidence；第26.21.23節與第26.21.25節分別記錄 ProcessNode→Duty及paired Duty→ProcessNode strict native pass，E1 aggregate為`Pass（evidence）`；第26.21.26節另補`1440×900`與`1024×768` viewport extension evidence。本ADR的正式QA-QC與repository integration均已通過；下方較早的`blocked／not-run`與candidate gate只作provenance。
...
- S7 RD handoff（2026-09-01／歷史）：E1-STRICT-NATIVE已由四個 minimum direction records完成，QA-QC-REVIEW已通過；當時下一個工作包為`E4-CANDIDATE-FREEZE`。該Git尾項其後已由repository integration閉合，不得依此歷史段落重新派工；若未來需新增DnD dependency、第二MIME／resolver／mutation owner、schema／API／permission、global listener或第二份evidence manifest，必須另開DEV回PM。
