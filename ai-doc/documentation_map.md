...
- 最新可執行規劃契約：`DEV-038` 流程－職掌－責任聯動規劃工作台，狀態為 `RD Implementation Ready / MVP Implementation In Progress / QA-QC Pending / Local Release Gate Pending`。核心仍是 `ProcessNode ↔ Duty ↔ DutyPositionRelation ↔ Position ↔ EmployeeAssignment` 與 OrganizationDocument V7 單一權威；MVP 已有 `/process-planning`、雙視角、Process／node 編輯與 edge 控制、Duty link、四 lane click／native drag、`ProcessOrganizationCanvas` 組織投影、keyboard placement、三向高亮與 draft reload persistence；typecheck、targeted `7 files／13 tests`、full `129 files／566 tests`、build（`370.97 kB gzip`）與六 viewport screenshot 均已保存。S6 API negative／invalid V7／409／provider isolation／reduced-motion／data-sanity 正式 QC gate仍待簽核。權威契約：`ai-doc/specs/DEV-038-process-duty-responsibility-planning-workbench.md`；ADR：`ai-doc/adr/ADR-008-process-planning-organization-version-authority.md`；摘要：`ai-doc/dev_task.md#dev-038流程職掌責任聯動規劃工作台`。
...
- `DEV-038`（`RD Implementation Ready / MVP Implementation In Progress / QA-QC Pending / Local Release Gate Pending`）：建立 CEO／主管共同使用的流程優先規劃層，左側同一 ProcessNode 的心智圖／流程圖、中間 Duty 橋接、右側 `ProcessOrganizationCanvas` 組織責任投影；既有 organization state 仍是資料權威。MVP 已落地 route、雙視角、Process／node 編輯與 edge 控制、Duty link、四 lane click／native drag、keyboard placement、三向高亮與 draft reload persistence；`1024～1279px` 收合右側投影並保留 Position selector fallback，`<1024px` 手機唯讀。ADR-008 的 OrganizationDocument V7 單一權威、Duty delete fail closed 與 DEV-034 relation reuse 不變。S6 正式 QC gate仍待API negative／invalid V7／409／provider isolation／reduced-motion／data-sanity證據簽核；不得deploy／release。權威契約：`ai-doc/specs/DEV-038-process-duty-responsibility-planning-workbench.md`；ADR：`ai-doc/adr/ADR-008-process-planning-organization-version-authority.md`；摘要：`ai-doc/dev_task.md#dev-038流程職掌責任聯動規劃工作台`。
