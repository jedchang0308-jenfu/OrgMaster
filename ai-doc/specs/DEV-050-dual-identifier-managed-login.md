...
| 正式環境 | production authority＝false；沒有 migration／provider／deploy 操作 | 直接走 DEV-040 的 001～011 release 套新 schema |
...
Release impact：app-local alias 行為與 OrgMaster session admission 改變，新增 app-only view／SELECT grant；沒有新 secret 或 provider write。正式 gate 必須先處理 exact 001～014 ledger、相容窗口、existing-session 拒絕影響與安全 rollback／fix-forward；本文件不修改 release authority。
