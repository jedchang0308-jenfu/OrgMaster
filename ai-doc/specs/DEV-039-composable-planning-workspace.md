...
> **DEV-042 intentional replacement notice（2026-09-02／Completed）**：DEV-039的split-tree／tab、每類一份panel、layout persistence、pin／close guard、panel ownership、canonical workspace route與single relation placement繼續是現行工程基線；DEV-042已完成移除快速Drawer與「在工作台開啟」promotion層，十個module由頂部launcher直接open-or-focus。session-owned唯一`openDetails`、既有route projection、`supportsCollapsibleDetail`及optional-context open intent已通過Browser QA／QC並隨`4e3b2ce`／`c8cc16f`整合。DEV-042主spec只保留完成契約與證據，不另設active candidate或merge尾項。

> **DEV-041 follow-on contract（2026-09-02／Completed）**：DEV-041已完成registered relation滑鼠來源表面、三態落點與owner-canvas auto-pan的intentional replacement；四向native、zero-mutation、兩個owner canvas及diagnostics均通過Browser QA／QC。DEV-039的single session、strict MIME、registered resolver、App mutation owner、domain／API／permission／persistence及keyboard相容基線不變。DEV-041已隨`4e3b2ce`／`c8cc16f`整合，不另設active candidate或merge尾項。

> **2026-09-01 candidate provenance（已由repository integration correction取代）**：舊工作樹曾以selective staging形成候選；當時文件記錄的`86510f4`已不在目前repository可達歷史。現行可稽核落點改以`164a779`、`4e3b2ce`及merge commit `c8cc16f`為準；不得再依本段派工candidate freeze或merge。

> **2026-09-01 Formal QA-QC 覆寫（整合前歷史）**：E1四個 minimum directions、E2五案`historyEvidence`與E3兩案已由同一份 evidence manifest正式覆核；targeted component `3 files／19 tests`、full regression `160 files／664 tests（1 skipped）`、`npx tsc --noEmit --pretty false`、`npm run build`、文件一致性、fixture archive與task-owned runtime cleanup均通過。當時E4只剩使用者／PM明確授權；其後repository integration已依最上方現行修正閉合。本段只保留QA／QC provenance。
...
- DEV-032、DEV-034、DEV-036、DEV-037、DEV-038 domain contracts／baselines：各模組內部行為與權限權威；其中DEV-034／038的固定UI驗收已由本交付取代。
