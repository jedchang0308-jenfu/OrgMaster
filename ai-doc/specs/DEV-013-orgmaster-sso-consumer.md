...
The ordinary production owner profile `config/release/dev040-orgmaster-independent-production-v3.json` now defaults `ORGMASTER_JENFU_SSO_HANDOFF_MODE=off`, permits only the controlled values `off／on`, and fixes the Platform broker origin to `https://jenfu-platform-prod-9536592944.asia-east1.run.app`. The owner runtime can add these fields safely to the prior production environment and rejects an unknown transition, missing legacy value, fixed-value drift, alternate origin, Secret drift, migration drift, or unrelated infrastructure drift before a release capsule can proceed. Every handoff-mode change, including initial default-off guard and rollback, requires `jenfu.dev013.l4-owner-transition-authorization.v1` plus `jenfu.dev013.l4-owner-transition-readiness.v1`, bound to owner, source revision, release ID, complete controlled environment, prior value and an immutable predecessor receipt. `deploy:production` reads the predecessor object, verifies its hash and accepts only a live source-bound `jenfu.dev013.l4-execution-authorization.v1` root or a released owner terminal before it creates authority receipts. It accepts only an exact `guard／activate／rollback` transition tuple through the DEV-013 arguments; the protected owner workflow remains the sole mutation path. Fresh DEV-040 owner tests, abort-controller tests, database-boundary check, full suite, and client／server build are required after this change.
