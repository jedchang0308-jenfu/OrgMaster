...
The ordinary production owner profile `config/release/dev040-orgmaster-independent-production-v3.json` defaults `ORGMASTER_JENFU_SSO_HANDOFF_MODE=off`, permits only the controlled values `off／on`, and fixes the Platform broker origin to `https://jenfu-platform-prod-9536592944.asia-east1.run.app`. Every handoff-mode change, including initial default-off guard and rollback, requires source-bound `jenfu.dev013.l4-owner-transition-authorization.v1` plus `jenfu.dev013.l4-owner-transition-readiness.v2`, bound to owner, source revision, release ID, complete controlled environment, prior value and an immutable predecessor receipt. `deploy:production` accepts a live `jenfu.dev013.l4-execution-authorization.v2` root whose human scope binds target, resources, allowed／forbidden actions, risk and expiry but contains no source revision, or a released owner terminal carrying `sequence-root.v2`. OrgMaster source SHA, migration-runner digest, 001–014 manifest, candidate and deployed revision remain machine-bound by the owner release capsule and receipts. A source change invalidates affected build／test／migration／candidate／provenance evidence and reruns those gates; it does not itself require renewed human authorization. Exact `guard／activate／rollback`, migration, live-data, IAM, traffic, rollback, canonical smoke, scope／target and destructive-operation gates remain unchanged.
