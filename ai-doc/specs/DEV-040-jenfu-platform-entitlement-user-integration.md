...
6. 先前v1授權綁定 source `e15121af6b579a339a109c1125214bf4d29624e8`，只保留為歷史receipt且不能作v2 sequence root。修正必須先完成本地 gates、commit／push與review；fresh source、migration-runner digest、001–014 manifest、artifact、candidate與deployed revision由owner release capsule／receipts精確綁定。human Production L4 authorization v2只綁target、resources、allowed／forbidden actions、risk與expiry；來源SHA改變會使受影響machine evidence失效並重跑，不因SHA本身要求重新取得人類授權。新的OrgMaster G2 predecessor仍必須綁定fresh sequence root與Platform已生效off/off guard的provider re-attestation；不得修改live guard只為重播G1。
