...
2026-09-10 shared-foundation handoff correction：shared foundation維持Platform單一apply authority，但Platform native readiness必將相同canonical foundation receipt鏡像到OrgMaster own release bucket，供owner intent與prepare讀取。Receipt保留Platform `sourceRevision`；`foundation`是唯一豁免OrgMaster source equality的prerequisite，OrgMaster source lock、infra、runtime與production data不得豁免，且app-infra必hard-join OrgMaster source與同一foundation manifest。缺mirror、bytes drift或豁免擴張均在provider write前FAIL。
