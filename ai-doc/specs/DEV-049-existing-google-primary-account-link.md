...
- 2026-09-21 production admission operator：`scripts/dev049-production-admission-runner.mjs`與其library／tests已嵌入既有`orgmaster-prod-migration-runner` image，並只允許由既有Job command override執行。Self-hashed operation固定exact production target、source revision／source bytes SHA-256、deadline、expected admission revision、三份consumer evidence reference／hash及GCS operation／receipt prefixes；runner重算`{platform, orgmaster} UNION active governance app IDs`後，才呼叫既有migrator-only attestation與CAS routine。Apply可由exact readback byte-stable replay；active-set、revision、source或evidence drift在mutation前fail closed。禁止人工SQL、新Job、runtime DDL與任何sibling `*_core` access；activation須先產生OrgMaster PASS receipt再交Platform，rollback則反向CAS。
...
專用 signer state固定為`tfstate-jenfu-platform-prod / dev-049/managed-directory/default.tfstate`。Terraform state新增`terraform_data.provenance`，綁定exact project／number／region、merged source revision、provider-readback foundation manifest SHA-256、具名operator、signer email與唯一read-only scope。`dev049-managed-directory-plan-gate`要求完整四地址集合：runtime data read、provenance、signer與signer-level Token Creator；只允許`read`、`create`、`no-op`，update／delete／replace或缺址均fail closed。Google Admin DWD grant仍由外部管理介面執行並readback，Terraform不建立key、Secret或project IAM。

Owner release finalize會發布source／artifact-bound `jenfu.dev014.consumer-conformance.v1`。DEV-049 admission operation因此新增每個consumer的source revision與artifact digest；runner在任何transaction前逐件讀回三個owner bucket物件並驗raw-object SHA-256、schema、app、source、artifact、guard及content hash。只有全部PASS才進入dynamic active-set、support revision、attestation與CAS；ref存在但內容漂移不得寫DB。跨專案完整順序見Platform [DEV-014 Production runbook](../../Jenfu-Platform/ai-doc/runbooks/DEV-014-production-protected-release.md)。
