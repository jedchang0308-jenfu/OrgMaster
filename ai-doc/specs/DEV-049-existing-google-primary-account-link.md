...
剩餘跨app驗收使用Platform受版控的Production fixture plan v2與`jenfu.dev014.login-production-completion.v2` finalizer。v2唯讀重用`employee-shijie / jedchang0308@jenfu.com.tw`既有Workspace active證據，只允許新建`dev014-fp-google`及`dev014-fp-number`兩個Cloud Identity Free-only／無Gmail帳號與各自標記的OrgMaster fixture Employee；不得修改任何其他既有帳號、Employee、binding、assignment或permission。Workspace pending兩格只可由Free pending實跑加上明示account-class-independent equivalence覆蓋，不可偽造browser PASS。缺任一direct identity、equivalence、LOGIN、AI-PDM、deny、rate／race／logout或cleanup evidence即拒絕完成；DEV-049不另造第二套fixture或completion authority。

兩個 Free-only 帳號及兩筆 OrgMaster fixture Employee 已於 2026-09-22 建立並讀回，Employee ID 分別固定為 `01a0c82b-11c6-77ab-887f-58df9d243e63` 與 `01a0c82b-372c-7d20-ba3b-6e3b892d2f63`，規劃員編固定為 `JFS9014／JFS9015`。後續員編與Directory link只可由 `scripts/dev014-production-login-fixture-runner.mjs` 的 `assign → link → readback` 相執行；operator 固定兩筆 target、既有 OrgMaster runtime／DWD readonly／migration Job、exact source與workspace revision及八小時期限。alias collision、非active link、Directory customer／stable ID／Email／etag drift或非預期既有mapping一律在confirm前停止；receipt只保存alias、Employee ID、員編、狀態、revision及識別值SHA-256。這項受控資料操作不增加schema、migration、IAM、Secret、service或traffic authority，也不放寬最終browser與cleanup gate。

Production assign已產生兩筆`APPLIED`及精確`EXISTING`重播receipt，但UI啟用時揭露`activationCheck`先以Employee當前`inactive`狀態拒絕「轉為active」本身，導致已具員編的fixture也無法進入既有assignment fence。修正後activation gate先驗exact workspace revision與Employee存在，再一律交由PostgreSQL `assert_employee_activation_v1`判斷assignment／legacy exemption；有JFS assignment才可啟用，無assignment仍回`allowed=false / correctionRequired=true`。此修正不放寬Employee target、workspace CAS或DB fence，也不直接修改workspace資料；新增兩個service tests覆蓋inactive→allowed與inactive→rejected。
