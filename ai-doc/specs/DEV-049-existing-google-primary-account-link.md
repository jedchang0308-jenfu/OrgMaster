...
- 2026-09-21 owner-native release handoff：`config/release/dev040-orgmaster-independent-production-v3.json`與validator已納入DEV-049五個Directory keys及兩個Platform caller keys；`ORGMASTER_MANAGED_IDENTITY_ENABLED=true`、`jenfu.com.tw`、dedicated signer與Platform runtime email／subject由profile固定，customer ID／delegated subject為兩個必要外部值。DEV-040 owner tests與QC各`63／63`、abort `6／6`、full regression `875 PASS／1 skipped`、DB boundary及雙build PASS。
